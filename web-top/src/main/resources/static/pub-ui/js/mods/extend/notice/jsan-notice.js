    ...
    layer.noticeMarker = function (option) {
        ...
        if(typeof that.properties.option.noticeWindow === "object" && that.properties.option.noticeWindow["type"] !== 2) {
            ...
            if(typeof that.properties.option.noticeWindow["classType"] === "object") {
                for(var tab in that.properties.option.noticeWindow["classType"]) {
            ...
            for(var tab in that.noticeWindow.classType) {
            ...
            that.addNews = function (option) {
                for(var i in option.content) {
                    var item = $("<div class='lay-jsan-notice-marker-item' notice-url = '" + option.content[i]["url"] + "'></div>");
                ...
                var lowKey = typeof option["lowKey"] === "undefined" ? false : option["lowKey"];
